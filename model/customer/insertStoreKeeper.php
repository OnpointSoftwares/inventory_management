<?php
	...
	if(isset($_POST['customerDetailsCustomerFullName'])){
		...
		$storeName = htmlentities($_POST['customerDetailsCustomerStoreName']);
		$address = htmlentities($_POST['customerDetailsCustomerPassword']);
		$password = htmlentities($_POST['customerDetailsCustomerAddress2']);
		...
		if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			$sql = 'INSERT INTO user(fullName, StoreId, password, username, status) VALUES(:fullName,:StoreId,:password,:username,:status)';
			$stmt = $conn->prepare($sql);
			$stmt->execute(['fullName' => $fullName,'StoreId'=>$storeName,'password'=>$password,'username'=>$username,'status'=>'active']);
